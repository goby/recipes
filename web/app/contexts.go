...
// MinusOperandsContext provides the operands minus action context.
type MinusOperandsContext struct {
	context.Context
	*goa.ResponseData
	*goa.RequestData
	Left  int
...
// NewMinusOperandsContext parses the incoming request URL and body, performs validations and creates the
// context used by the operands controller minus action.
func NewMinusOperandsContext(ctx context.Context, r *http.Request, service *goa.Service) (*MinusOperandsContext, error) {
	var err error
	resp := goa.ContextResponse(ctx)
	resp.Service = service
	req := goa.ContextRequest(ctx)
	req.Request = r
	rctx := MinusOperandsContext{Context: ctx, ResponseData: resp, RequestData: req}
	paramLeft := req.Params["left"]
	if len(paramLeft) > 0 {
		rawLeft := paramLeft[0]
		if left, err2 := strconv.Atoi(rawLeft); err2 == nil {
...
// OK sends a HTTP response with status code 200.
func (ctx *MinusOperandsContext) OK(resp []byte) error {
	ctx.ResponseData.Header().Set("Content-Type", "text/plain")
	ctx.ResponseData.WriteHeader(200)
	_, err := ctx.ResponseData.Write(resp)
	return err
