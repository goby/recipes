...
type (
	...
	// MinusOperandsCommand is the command line data structure for the minus action of operands
	MinusOperandsCommand struct {
		// Left operand
		Left int
		// Right operand
		Right       int
...
func RegisterCommands(app *cobra.Command, c *client.Client) {
	...
	command = &cobra.Command{
		Use:   "minus",
		Short: `minus returns the result of the left and right parameters in the response body`,
	}
	tmp2 := new(MinusOperandsCommand)
	sub = &cobra.Command{
		Use:   `operands ["/minus/LEFT/RIGHT"]`,
		Short: ``,
...
// Run makes the HTTP request corresponding to the MinusOperandsCommand command.
func (cmd *MinusOperandsCommand) Run(c *client.Client, args []string) error {
	var path string
	if len(args) > 0 {
		path = args[0]
	} else {
		path = fmt.Sprintf("/minus/%v/%v", cmd.Left, cmd.Right)
	}
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
	resp, err := c.MinusOperands(ctx, path)
	if err != nil {
		goa.LogError(ctx, "failed", "err", err)
		return err
	}
...
// RegisterFlags registers the command flags with the command line.
func (cmd *MinusOperandsCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	var left int
	cc.Flags().IntVar(&cmd.Left, "left", left, `Left operand`)
	var right int
	cc.Flags().IntVar(&cmd.Right, "right", right, `Right operand`)
